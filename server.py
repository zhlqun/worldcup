    ...
    for line in open("sina_dynamic/data/worldcupDetail", "r"):
        ...
        if context[0] == "text":
            if line[4] == "1" or line[4] == 1:
                ...
                ret_str += """<p><font size="6" color="red">%s | %s | %s %s</font></p>""" % (line[0], context[1], line[3], line[2])
            else:
                ret_str += """<p><font size="6" color="black">%s | %s | %s %s</font></p>""" % (line[0], context[1], line[3], line[2])
        elif context[0] == "url":
            ret_str += """<div align="center">
			<img alt="" src="%s" align="middle" style="margin: 0 auto" width="800" height="600"/></div>""" % (context[1])   
        i += 1
        if i > 400:
            break
    return ret_str
